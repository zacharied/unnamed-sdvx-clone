...
public:
	...
	shared_ptr<Mesh> fullscreenMesh;
	shared_ptr<Material> fullscreenMaterial;
	shared_ptr<Texture> backgroundTexture;
	shared_ptr<Texture> frameBufferTexture;
...
class TestBackground : public FullscreenBackground
{
	virtual bool Init(bool foreground) override
	{
		...
		if (foreground)
		{
			auto fbtex = Texture::Create(g_resolution);
			assert(fbtex);
			frameBufferTexture = std::move(*fbtex);
	...
	unique_ptr<Material> LoadBackgroundMaterial(const String& path)
	{
		String skin = g_gameConfig.GetString(GameConfigKeys::Skin);
