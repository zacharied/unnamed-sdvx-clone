...
struct Label
{
	...
	int size;
	float scale;
	IFont::TextOptions opt;
	shared_ptr<IFont> font;
	String content;
};
...
struct ImageAnimation
{
	...
	float Timer;
	bool LoadComplete;
	Vector<unique_ptr<IImage>> Frames;
	Thread* JobThread;
	lua_State* State;
};
...
struct GUIState
{
	...
	Map<String, shared_ptr<IFont>> fontCahce;
	Map<lua_State*, Set<int>> vgImages;
	shared_ptr<IFont> currentFont;
	Vector4 fillColor;
	int textAlign;
	int fontSize;
	shared_ptr<IMaterial> fontMaterial;
	shared_ptr<IMaterial> fillMaterial;
...
static void AnimationLoader(Vector<FileInfo> files, ImageAnimation* ia)
{
	...
	{
		auto newframe = Image::Create(files[i].fullPath);
		if (!newframe)
			return;
		ia->Frames.push_back(std::move(*newframe));
	}
...
static int lUpdateLabel(lua_State* L /*int labelId, const char* text, int size*/)
{
	...
	Label& toUpdate = g_guiState.textCache[L][labelId];
	toUpdate.text = g_guiState.currentFont->CreateText(Utility::ConvertToWString(text), size * g_guiState.t.GetScale().y, toUpdate.opt);
	toUpdate.size = size;
	toUpdate.scale = g_guiState.t.GetScale().y;
	toUpdate.content = text;
	return 0;
}
...
static int lDrawLabel(lua_State* L /*int labelId, float x, float y, float maxWidth = -1*/)
{
	...
	if (fabsf(te.scale - g_guiState.t.GetScale().y) > 0.001)
	{
		...
		te.text = te.font->CreateText(Utility::ConvertToWString(te.content), Math::Round((float)te.size * te.scale), te.opt);
	}
	float mwScale = 1.0f;
	if (maxWidth > 0)
	{
...
static int lFastRect(lua_State* L /*float x, float y, float w, float h*/)
{
	...
	w = luaL_checknumber(L, 3);
	h = luaL_checknumber(L, 4);
	unique_ptr<IMesh> quad = Mesh::GenerateQuad(Vector2(x, y), Vector2(w, h));
	MaterialParameterSet params;
	params.SetParameter("color", g_guiState.fillColor);
	g_guiState.rq->DrawScissored(g_guiState.scissor, g_guiState.t, quad.get(), g_guiState.fillMaterial.get(), params);
	return 0;
}
...
static int lFastText(lua_State* L /* String utf8string, float x, float y */)
{
	...
	unique_ptr<IText> te = g_guiState.currentFont->CreateText(text, g_guiState.fontSize, IFont::TextOptions::None);
	Transform textTransform = g_guiState.t;
	textTransform *= Transform::Translation(Vector2(x, y));
...
static int lFastTextSize(lua_State* L /* char* text */)
{
	...
	WString text = Utility::ConvertToWString(s);
	unique_ptr<IText> l = g_guiState.currentFont->CreateText(text, g_guiState.fontSize, IFont::TextOptions::None);
